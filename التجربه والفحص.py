...
engine = create_engine("you.db")
BASE.metadata.bind = engine
BASE.metadata.create_all(engine)


class youthon_GlobalCollection(BASE):
    __tablename__ = "youthon_globalcollection"
    ...
    def __repr__(self):
        return "<youthon Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
...
youhon_GlobalCollection.__table__.create(checkfirst=True)
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(
            youthon_GlobalCollection.keywoard).distinct().all()
...
@youthon.on(events.NewMessage(outgoing=True, pattern=r"\.تحديث"))
async def _(event):
